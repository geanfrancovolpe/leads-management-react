...
import { PromptLibrary } from '@/types/prompt';
...
export default function PromptsListPage() {
  ...
  const fetchPrompts = async () => {
    try {
      setLoading(true);
      const response = await promptsService.getPrompts(search);
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 mt-2">Manage your prompt templates</p>
          </div>
              ...
              {loading ? (
                ...
                prompts.map((prompt) => (
                  <TableRow key={prompt.id}>
                    <TableCell className="font-medium">{prompt.template_name || '-'}</TableCell>
                    <TableCell>{prompt.sequence_phase}</TableCell>
                    <TableCell>{prompt.sequence_step}</TableCell>
                    <TableCell>{prompt.purpose}</TableCell>
                    <TableCell>{prompt.campaign_name || '-'}</TableCell>
